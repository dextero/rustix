...
bitflags! {
    ...
    pub struct MprotectFlags: u32 {
        ...
        /// `PROT_BTI`
        #[cfg(all(linux_kernel, feature = "linux-raw-sys", target_arch = "aarch64"))]
        const BTI = linux_raw_sys::general::PROT_BTI;
        /// `PROT_MTE`
        #[cfg(all(linux_kernel, feature = "linux-raw-sys", target_arch = "aarch64"))]
        const MTE = linux_raw_sys::general::PROT_MTE;
        /// `PROT_SAO`
        #[cfg(all(linux_kernel, feature = "linux-raw-sys", any(target_arch = "powerpc", target_arch = "powerpc64")))]
        const SAO = linux_raw_sys::general::PROT_SAO;
        /// `PROT_ADI`
        #[cfg(all(linux_kernel, feature = "linux-raw-sys", any(target_arch = "sparc", target_arch = "sparc64")))]
